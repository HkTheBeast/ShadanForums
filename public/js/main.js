...
// ===== NUCLEAR RESPONSIVE DROPDOWN FIX =====
function initNuclearDropdowns() {
  console.log('🚀 INITIALIZING NUCLEAR DROPDOWNS');
  
  const dropdowns = document.querySelectorAll('.dropdown');
  console.log('Found', dropdowns.length, 'dropdowns');
  
  // NUCLEAR APPROACH: Remove ALL href attributes and use pure JavaScript
  dropdowns.forEach(dropdown => {
    const toggle = dropdown.querySelector('.dropdown-toggle');
    const content = dropdown.querySelector('.dropdown-content');
    
    if (!toggle || !content) {
      console.log('❌ Missing toggle or content');
      return;
    }
    
    console.log('🔧 Processing dropdown:', toggle.textContent);
    
    // STEP 1: Remove href completely to prevent any navigation
    toggle.removeAttribute('href');
    toggle.style.cursor = 'pointer';
    
    // STEP 2: Remove any existing event listeners by cloning
    ...
    // STEP 3: Add fresh click handler
    ...
    // STEP 4: Close dropdown when clicking on links
  ...
  // STEP 5: Close dropdowns when clicking outside
  ...
  console.log('✅ NUCLEAR DROPDOWNS INITIALIZED');
}

// Initialize multiple times to catch all scenarios
initNuclearDropdowns();
document.addEventListener('DOMContentLoaded', initNuclearDropdowns);
window.addEventListener('load', initNuclearDropdowns);

// Re-initialize on resize
window.addEventListener('resize', function() {
  setTimeout(initNuclearDropdowns, 100);
});
